...
class ReservationSystemTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.data_dir / "hotels.json").write_text(
            json.dumps(
                [{"hotel_id": "H1", "name": "Hotel One", "rooms_total": 5, "rooms_available": 5}],
                indent=2,
            ) + "\n",
            encoding="utf-8",
        )
        ...
        (self.data_dir / "reservations.json").write_text("[]\n", encoding="utf-8")
    ...
    def test_hotel_crud(self) -> None:
        ...
        updated = Hotel.modify_hotel_information(self.data_dir, "H2", rooms_available=2)
    ...
    def test_customer_crud(self) -> None:
        created = Customer.create_customer(self.data_dir, "C2", "Bob", "bob@example.com")
        ...
        updated = Customer.modify_customer_information(self.data_dir, "C2", name="Bobby")
    ...
    def test_reservation_create_and_cancel_updates_inventory(self) -> None:
        ...
        res = Reservation.create_a_reservation(self.data_dir, "R1", "C1", "H1", room_count=2)
    ...
    def test_conflict_and_not_found_paths(self) -> None:
        ...
        with self.assertRaises(NotFoundError):
            Reservation.create_a_reservation(self.data_dir, "R9", "NO_CUST", "H1", 1)

        with self.assertRaises(ConflictError):
            Reservation.create_a_reservation(self.data_dir, "R2", "C1", "H1", room_count=999)

        _ = Reservation.create_a_reservation(self.data_dir, "R3", "C1", "H1", room_count=1)
        with self.assertRaises(ConflictError):
            Reservation.create_a_reservation(self.data_dir, "R3", "C1", "H1", room_count=1)
    ...
    def test_prevent_delete_if_active_reservation(self) -> None:
        _ = Reservation.create_a_reservation(self.data_dir, "R10", "C1", "H1", room_count=1)
    ...
    def test_invalid_json_prints_error_and_continues(self) -> None:
        (self.data_dir / "hotels.json").write_text("{ invalid json", encoding="utf-8")
    ...
    def test_invalid_rows_are_skipped(self) -> None:
        (self.data_dir / "customers.json").write_text(
            json.dumps(
                [
                    {"customer_id": "C_OK", "name": "Ok", "email": "ok@example.com"},
                    {"customer_id": "C_BAD", "name": "Bad", "email": "not-an-email"},
                ],
                indent=2,
            ) + "\n",
            encoding="utf-8",
